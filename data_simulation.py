...
# print(u'\u03C3') # sigma
# %%
from functions import spectrum, fitter
...
from functions import pre_lst_sqrs_adjstmt, fitter, gen_a_dot_array
#%%
...
def execute_fit(sim_data, fit_specs, R):
    ...
    n_s = n_fac * n_days
    fit_list = fitter(sim_data, p_list, np.zeros(len(p_list)), n_s, 5, ω, 1)
    fit_data = fit_list[0]
...
def show_fit_old(sim_data, fit_specs, e_fac_bar, e_fac_slope, show_list, zoom_list, big_fit_list,
    ...
    if (show_list[5] == 1): # show decrease
        ...
        #"""
        plt.xlabel("t [d]", fontsize = 20)
        plt.ylabel(r"$\dot{\bar{a}}$ [$\frac{m}{d}$]", fontsize = 20)
        ...
        plt.tick_params(axis = 'both', which = 'major', labelsize = 15)
        plt.legend(fontsize = 17.5, labelspacing = 0.25, loc = 4, markerscale = 3)
        plt.grid()
        if (save_on == 1):
            type_fig = "dec_" 
            name = path + type_fig + settings + ".png"
...
def show_fit(sim_data, fit_specs, e_fac_bar, e_fac_slope, show_list, zoom_list, big_fit_list,
    ...
    if (show_list[0] == 1): # show spectrum data and bar
        ...
        plt.loglog(p_o * to_min, a_o,
                   'saddlebrown', ls = 'solid', alpha = 1, lw = 1,
                   label = r'$a_{\text{Sim}}$')
    ...
    if (show_list[3] == 1): # show slope
        ...
        ax.get_yaxis().get_major_formatter().set_useMathText(True)
        ax.set_xlabel(r'$t$ [d]', fontsize = 20)
        ax.set_ylabel(r'$\dot{a}$ [md$^{-1}$]', fontsize = 20)
        ax.set_ylim(zoom_list[0], zoom_list[1])
        ax.tick_params(axis = 'both', which = 'major', labelsize = 15)
        fig.legend(fontsize = 20, labelspacing = 0.5, ncols = 1, columnspacing = 0.5,
                   loc = 4, bbox_to_anchor = (1, 0),bbox_transform = ax.transAxes)
        ax.grid()
        if (save_on == 1):
            type_fig = "slope_" 
...
status_p_t = 1 # time dependence of main periods
status_hf = 1 # additional hf periods
status_lf = 1 # additional lf periods
...
path = os.getcwd() + "/Thesis/LATEX/Images/Simulation_new/" + path_status
save_specs = [0, path]
#%%
