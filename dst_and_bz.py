...
year = '2023'
image_path = 'Thesis/LATEX/Images/Results/Additional/DST_BZ' + year

foldername_dst = 'dst and ae'
foldername_bz = 'magnetometer/2018'
...
vline_list_specs1 = ['gold', 1, 1, "CME", (0, (4, 4))]
#%%
...
def ydhm_to_mjd(ydhm_list):
    #ydhm_list = [yyyy, ddd, hh, mm]
    yyyy = ydhm_list[0]
    ddd = ydhm_list[1]
    hh = ydhm_list[2]
    mm = ydhm_list[3]
    string = '%d:%d:%d:%d:00.000' % (yyyy, ddd, hh, mm)
    t_obj = Time(string, format = 'yday')
    t_mjd = t_obj.mjd
    return(float(t_mjd))

def bz_array_mjd(array):
    bz_list = array[:, -1]
    ydhm_array = array[:, : 4]
    mjd_list = np.array([])
    for i in range(0, len(bz_list)):
        ydhm_list = ydhm_array[i]
        mjd = ydhm_to_mjd(ydhm_list)
        mjd_list = np.append(mjd_list, mjd)
    new_array = np.vstack((mjd_list, bz_list)).T
    data = np.array([[0, 0]])
...
dst_data = np.loadtxt(foldername_dst + '/dst_master.txt')
bz_data = np.loadtxt(foldername_bz + '/dscovr_hr.lst.txt')
bz_data = bz_array_mjd(bz_data)


dst_data = array_modifier(dst_data, MJD_0, n_days_tot)
bz_data = array_modifier(bz_data, MJD_0, n_days_tot)
#%%
...
def plot_func_6_ind(data1, specs1, y_label_specs1,
                    ...
                    tit, fosi, n_cols, location, anchor,
