...
interval_list_el = [[0.0205,0.023], [0.045, 0.1], [0.015, 0.018]]
...
for i in range(0,1):
    ...
    ylimits = [0, 0]
    ylimits = [1, 1000]
    #[0.0205,0.023], [0.045, 0.1], [0.015, 0.018]
    v_line_list = [0.42, 0.45]
    print("gut")
    ...
    if (lsa_method == 1): # give t0 and periods; fit h, s, amplitudes and phases
        ...
        for q_tilde in range(0, Q_tilde + 1):
            ...
            for n in range(1, n_s + 1):
                ...
                d_slope = (da_q_n - da_q_n_1) / Δτ
                if (q_tilde >= 1 and q_tilde <= R): # μ
                    r = q_tilde
                    p_r = p_list[r - 1]
                    slope = slope * np.sin(2 * np.pi / p_r * t)
                    d_slope = d_slope * np.sin(2 * np.pi / p_r * t)
                elif (q_tilde >= R + 1 and q_tilde <= 2 * R): # η
                    r = q_tilde - R
                    p_r = p_list[r - 1]
                    slope = slope * np.cos(2 * np.pi / p_r * t)
                    d_slope = d_slope * np.cos(2 * np.pi / p_r * t)
                # else: vec = vec # a_bar
                #if (d_slope < 0):
                #    print("---------------------------------------------")
                #    print("q_tilde = %2d | n = %3d | d_slope = %.10f" % (q_tilde, n, d_slope))
                #    print("da_q_n = %.10f | da_q_n_1 = %.10f" % (da_q_n, da_q_n_1))
                sub_list.append(slope)
                e_sub_list.append(d_slope)
            para_list.append(sub_list)
            para_e_list.append(e_sub_list)
            print("asdhjshfjshf")
            print(len(para_list[0]), len(para_e_list[0]))
...
t_fit_a = tt.time()
configuration_list = [[8, 3, 10000, 0],
                 [8, 3, 10000, 1]]
...
def fft_logplot_spec_3(p_o_list, a_o_list, data_spec_array,
    ...
    for i in range(1, n_data):
        new_a_o = a_o_list[i] / log_fac_list[i-1]
    ...
    for i in range(0, n_marker):
        plt.scatter(per_list_list[i], new_amp_list_list[i],
                    color = marker_spec_array[i][2], s = 10,
        ...
        for j in range(0, len(per_list_list[i])):
            ...
            plt.annotate(string, (per_list_list[i][j], new_amp_list_list[i][j]),
                         ...
                         fontsize = 10)
            str1 = ' = '
            str2 = str(round(per_list_list[i][j], 3))
            str3 = " d"
            plt.figtext(0.92, 0.875 - k, string + str1 + str2 + str3,
                        fontsize = 10, ha = 'left')
            k += 0.05
        k += 0.03
...
spectrum_data_specs = [[0.75, "r", el_symb],
                       [0.75, col_list[0], lab_gen(configuration_list[0])],
                       [0.75, col_list[1], lab_gen(configuration_list[1])]]
spectrum_per = [el_per]
spectrum_amp = [el_amp]
...
xlimits = [0, 0]
ylimits = [0, 0]
spectrum_tit_1 = name + " data: spectrum of " + el_symb + " and " + el_tilde
...
for i in range(0, len(configuration_list)):
    ...
    """
    configuration0b = "all"
    path_add = configuration0a + configuration0b + configuration1 + configuration2 + configuration3
...
path_add = "0"
decr_bar_new(slope_data_list, slope_specs_list, e_fac,
             add_data_list, add_data_specs_list,
             el_label, el_unit,
             flx_data, flx_spec_list,
